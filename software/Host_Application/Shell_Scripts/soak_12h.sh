#!/bin/sh
# 24-hour soak — ST-08b / NFR-003
# Identical to soak_2h.sh but runs for 86400 s instead of 7200 s so
...
# ---- boot ---------------------------------------------------------
echo "# 24-hour soak — started $(isots)" > "$LOG"
...
echo petalinux | sudo -S nohup python3 /home/petalinux/server.py \
  > ${LOGDIR}/soak24_server_${TS}.log 2>&1 &
echo "# server.log=${LOGDIR}/soak24_server_${TS}.log" >> "$LOG"
sleep 10
